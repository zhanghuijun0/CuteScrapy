...
MAIL_USE_TLS = False
MAIL_USE_SSL = True
MAIL_NAME = 'Mail Alert'


def __format_addr(self, s):
    name, addr = email.Utils.parseaddr(s)
    return email.Utils.formataddr(
        (email.header.Header(name, 'utf-8').encode(), addr.encode('utf-8') if isinstance(addr, unicode) else addr))
...
if __name__ == '__main__':
    sendEmail(html='<h1>hello</h1><h2> world!</h2>',
              subject='test1',
              mail_to_name='test',
              # mail_to=['zhanghuijun0@126.com'],files=[open('mail.py','r')])
              mail_to=['1483386979@qq.com'], files=[])
